...
def run_sync() -> dict:
    ...
    依序拉取 6 類報表 + 寫入每日快照，全部寫入 Supabase 快取。

    Returns:
        包含同步結果的字典:
        { status: str, message: str, updated_reports: list[str], duration_seconds: float }

    Raises:
        Exception: 當任一報表拉取或寫入失敗時拋出
    """
    start_time = datetime.now()
    updated_reports = []

    # NOTE: 定義報表類型與對應的取得方法，方便維護與擴展
    report_tasks = [
    ...
    for report_type, fetch_fn in report_tasks:
        logger.info(f"同步: {report_type}")
        data = fetch_fn()
        supabase_service.upsert_cache(report_type, data)
        updated_reports.append(report_type)

    # 將當日 KPI 寫入每日快照表，供前端趨勢圖使用
    logger.info("同步: 每日快照")
    overview_data = supabase_service.get_cache("overview")
    ...
    logger.info(f"✅ 同步完成，耗時 {duration:.1f} 秒，更新報表: {updated_reports}")

    return {
        "status": "success",
        "message": f"資料同步成功（耗時 {duration:.1f} 秒）",
